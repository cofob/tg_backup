...
fn query_stdio_capabilities(
    ...
    'out: loop {
        let mut charbuf = [0u8; 4096];
        // No detached reader may outlive capability negotiation and steal UI input.
        #[cfg(not(windows))]
        {
            let remaining = deadline.saturating_duration_since(std::time::Instant::now());
            if remaining.is_zero() { return Err(Errors::NoStdinResponse); }
            let timeout = rustix::event::Timespec::try_from(remaining)
                .map_err(|_| Errors::NoStdinResponse)?;
            let stdin = io::stdin();
        ...
        if read == 0 { return Err(Errors::NoStdinResponse); }
...
fn query_with_timeout(
    ...
    restore()?;
    result?;
    rx.try_iter().find_map(|message| match message {
        QueryResult::Done(result) => Some(result),
        QueryResult::Busy => None,
    }).ok_or(Errors::NoStdinResponse)
}
